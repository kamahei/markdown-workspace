# コード・数式・図

いずれも、ドキュメントが実際にそれを含んでいるときだけ読み込まれます。
普通の文章だけのページは、どれも読み込みません。

## シンタックスハイライト

```typescript
interface FileSource {
  readonly kind: 'handle' | 'file-url' | 'snapshot';
  readonly canPersist: boolean;
  readonly canRefresh: boolean;

  readFile(path: string): Promise<FileContent>;
  listDirectory(path: string): Promise<DirectoryEntry[]>;
}
```

```python
def slugify(text: str) -> str:
    """見出しアンカーは再描画しても変わってはいけない。"""
    return re.sub(r"[^\w\s-]", "", text.lower()).strip().replace(" ", "-")
```

```bash
pnpm install
pnpm dev          # 拡張機能を読み込んだ Chrome を起動
pnpm test         # ユニットテスト
```

未知の言語はエラーにせず、そのまま表示します:

```notalanguage
書いたとおりに、そのまま残ります
```

## 数式

文中に収まるインライン数式: $E = mc^2$ のように自然に並びます。

ディスプレイ数式は独立した行になります:

$$
\frac{\partial L}{\partial \theta} = \sum_{i=1}^{n} (y_i - \hat{y}_i) x_i
$$

不正な数式はソースを残します — 打ち間違いのせいで書いた内容が消えるほうが、
はるかに悪い失敗だからです。

## 図

```mermaid
graph TD
    accTitle: ドロップ時の処理
    accDescr: フォルダならファイルツリーを開き、Markdown ファイルなら描画します。いずれもサイドバーに兄弟ファイルが並び、文書間のリンクはその場で移動します。
    A[フォルダを Chrome にドロップ] --> B{何が来た?}
    B -->|.md ファイル| C[描画する]
    B -->|フォルダ| D[ファイルツリーを表示]
    C --> E[サイドバーに同階層を表示]
    D --> E
    E --> F[相対リンクはその場で移動]
```

```mermaid
sequenceDiagram
    participant U as あなた
    participant R as リーダー
    participant W as Service Worker
    U->>R: notes.md を開く
    R->>R: 本文を即座に描画
    R->>W: フォルダ一覧を要求
    W-->>R: ファイルとディレクトリ
    R->>U: サイドバーが表示される
```

解析できない図は、エラーとともにソースを表示します。ドキュメントの他の部分は
影響を受けません。

最初の図には `accTitle` と `accDescr` を書いています。スクリーンリーダーは
図を1つの画像として読み上げます — ラベルを個別に読むと DOM 順になり、
フロー図の流れとは一致しないからです。この2行が無い場合は図のソースが
代替として提示されます。どのノードからどこへ進むかは伝わりますが、
自分で書いた一文のほうが良いです。
